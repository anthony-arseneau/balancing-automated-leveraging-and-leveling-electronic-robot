...
//Modifications by Tanner Altenkirk, Anthony Arseneau, Sawyer Stanley
//December 1, 2023
//--------------------------------------------------------------
...
//stepper motors
AccelStepper stepperA(1, 3, 4);  //(driver type, STEP, DIR) Driver A
AccelStepper stepperB(1, 5, 6);  //(driver type, STEP, DIR) Driver B
AccelStepper stepperC(1, 7, 8);  //(driver type, STEP, DIR) Driver C
...
int ENA = 2;                                           //enable pin for the drivers
double angOrig = 206.662752199;                        //original angle that each leg starts at
double speed[3] = { 0, 0, 0 }, speedPrev[3], ks = 500;  //the speed of the stepper motor and the speed amplifying constant
...
//PID variables
double kp = 1.5E-4, ki = 1.5E-6, kd = 4.1E-3;                                                       //PID constants
...
void setup() {
  Serial.begin(115200);
  // Setting starting positions to 0
...
void moveTo(double hz, double nx, double ny) {
  ...
  if (detected) {
    ...
    stepperA.setAcceleration(speed[A] * 300);
    stepperB.setAcceleration(speed[B] * 300);
    stepperC.setAcceleration(speed[C] * 300);
...
void PID(double setpointX, double setpointY) {
  ...
  if (p.x != 0) {
    ...
    for (int i = 0; i < 3; i++) {
      ...
      speed[i] = constrain(speed[i], 0, 2500);                                                                                           //constrains sped from 0 to 1000
    }
  ...
  //if the ball is not detected (the x value will be 0)
    ...
    if (p.x == 0 /*&& p.y == 0*/) {             //if the ball is still not detected
